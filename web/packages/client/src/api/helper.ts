...
export function handleApiError(error: unknown): ApiResponse<null> {
  if (axios.isAxiosError(error)) {
    ...
    const errorCode = HttpStatusToErrorCode[status] ?? ErrorCode.INTERNAL_SERVER_ERROR;
  ...
  // Non-Axios errors
  return apiResponse<null>(false, ErrorCode.INTERNAL_SERVER_ERROR, null, 'Unexpected error');
}
