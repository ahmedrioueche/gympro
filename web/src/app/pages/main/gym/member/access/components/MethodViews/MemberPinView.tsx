...
interface MemberPinViewProps {
  pinCode?: string;
  isFullscreen: boolean;
  setIsFullscreen: (v: boolean) => void;
}

export const MemberPinView: React.FC<MemberPinViewProps> = ({
  pinCode,
  isFullscreen,
  setIsFullscreen,
}) => {
      ...
      >
        ...
        <div className="flex flex-col items-center gap-8">
          <div className="p-6 bg-primary/10 rounded-3xl">
            <Hash className="w-16 h-16 md:w-24 md:h-24 text-primary" />
          </div>
          <h2 className="text-7xl md:text-9xl font-black text-zinc-900 tracking-[0.4em] font-mono">
            {pinCode}
          </h2>
        </div>
  ...
  return (
    <div
      onClick={() => pinCode && setIsFullscreen(true)}
      className={cn(
    ...
    >
      ...
      <div className="space-y-2 text-center">
        ...
        <h2 className="text-4xl md:text-5xl font-black text-white tracking-[0.3em] font-mono">
          {pinCode || "••••••"}
        </h2>
      </div>
      {!pinCode && (
